...
module.exports = class JSONAPISerializer {
  ...
  serializeRelationship(rType, rData, rOptions, included, extraData) {
    ...
    if (_.has(rData, 'data')) {
      if (_.has(rData, 'data.id') && _.has(rData, 'data.attributes')) {
        const oryg = Object.assign({}, rData);
        rData = oryg.data.attributes;
        rData[rOptions.id] = oryg.data.id;
      } else if (Array.isArray(rData.data)) {
        const newRdata = [];
        _.forEach(rData.data, (d) => {
          newRdata.unshift({"data": d});
        });
        rData = newRdata;
      }
    }
